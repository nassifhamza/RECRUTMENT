    ...
    if (!empty($_POST["name"]) && !empty($_POST["email"]) && !empty($_POST["password"]) && !empty($_POST["prenom"])  && !empty($_POST["USERNAME"]) && !empty($_POST["password1"])) {
        ...
        if ($num == 0) {
            if ($pass === $pass1) {
                ...
                echo "<div style='border-radius:20px;color:black;margin-top:20px;background-color:red;padding:20px;font-weight:bolder;font-size:larger;text-transform:uppercase'>verify your password please,they seem are not VALID </div>";
            }
        } else {
